...
public class WalkNodeProcessorBetter extends NodeProcessor
{
        ...
        for (int i = 0; i < xSize; ++i)
        {
            for (int j = 0; j < ySize; ++j)
            {
                for (int k = 0; k < zSize; ++k)
                {
                    ...
                    if ((pathnodetype == PathNodeType.DOOR_WOOD_CLOSED || pathnodetype == PathNodeType.DOOR_IRON_CLOSED) && canOpenDoorsIn && canEnterDoorsIn)
                    {
