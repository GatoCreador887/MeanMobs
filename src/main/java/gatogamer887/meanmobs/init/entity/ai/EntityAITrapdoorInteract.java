...
public abstract class EntityAITrapdoorInteract extends EntityAIBase
{
    protected EntityLiving entity;
    protected BlockPos doorPosition = BlockPos.ORIGIN;
    /** The wooden door block */
    protected BlockTrapDoor doorBlock;
    /** If is true then the Entity has stopped Door Interaction and compoleted the task. */
    boolean hasStoppedDoorInteraction;
    float entityPositionX;
    float entityPositionZ;

    public EntityAITrapdoorInteract(EntityLiving entityIn)
    {
        this.entity = entityIn;

        if (!(entityIn.getNavigator() instanceof PathNavigateGround))
        {
        	throw new IllegalArgumentException("Unsupported mob type " + EntityList.getKey(entityIn) + " for TrapdoorInteractGoal");
        }
    }
    ...
    private BlockTrapDoor getBlockDoor(BlockPos pos)
    {
        IBlockState iblockstate = this.entity.world.getBlockState(pos);
        Block block = iblockstate.getBlock();
        return block instanceof BlockTrapDoor && (iblockstate.getMaterial() == Material.WOOD || iblockstate.getMaterial() == Material.IRON) ? (BlockTrapDoor)block : null;
    }
}