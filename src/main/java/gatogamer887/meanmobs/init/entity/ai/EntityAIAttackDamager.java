...
public class EntityAIAttackDamager extends EntityAITarget
{
    private final boolean entityCallsForHelp;
    /** Store the previous revengeTimer value */
    private int revengeTimerOld;
    private final Class<?>[] excludedReinforcementTypes;
    private final Class<?>[] otherReinforcementTypes;

    public EntityAIAttackDamager(EntityCreature creatureIn, boolean entityCallsForHelpIn, Class[] excludedReinforcementTypes, Class<?>... otherReinforcementTypes)
    {
        super(creatureIn, true);
        this.entityCallsForHelp = entityCallsForHelpIn;
        this.excludedReinforcementTypes = excludedReinforcementTypes;
        this.otherReinforcementTypes = otherReinforcementTypes;
        this.setMutexBits(1);
    }
    ...
    protected void alertOthers()
    {
        ...
        for (Class<?> clazz : this.otherReinforcementTypes)
        {
        	if (EntityCreature.class.isAssignableFrom(clazz))
        	{
        		for (EntityCreature entitycreature : this.taskOwner.world.getEntitiesWithinAABB((Class<? extends EntityCreature>) clazz, (new AxisAlignedBB(this.taskOwner.posX, this.taskOwner.posY, this.taskOwner.posZ, this.taskOwner.posX + 1.0D, this.taskOwner.posY + 1.0D, this.taskOwner.posZ + 1.0D)).grow(d0, 10.0D, d0)))
                {
                    if (this.taskOwner != entitycreature && entitycreature.getAttackTarget() == null && (!(this.taskOwner instanceof EntityTameable) || ((EntityTameable)this.taskOwner).getOwner() == ((EntityTameable)entitycreature).getOwner()) && !entitycreature.isOnSameTeam(this.taskOwner.getRevengeTarget()))
                    {
                        boolean flag = false;
